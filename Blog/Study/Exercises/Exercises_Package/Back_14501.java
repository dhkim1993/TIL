...
public class Back_14501 {
    public static void main(String[] args) throws Exception {
        BufferedReader br = new BufferedReader(new InputStreamReader(new FileInputStream("/Users/voldemort/IdeaProjects/TIL/Blog/Study/Exercises/input.txt")));
        ...
        for (int i = 1; i <= N + 1; i++) {
            ...
            System.out.println("dp["+i+"] : " + dp[i] + ", max: " + max);
            dp[T[i] + i] = Math.max(dp[T[i] + i], P[i] + dp[i]);
            //출력될 최대 수입
            System.out.println("dp["+dp[T[i]+i]+"] : " + dp[T[i] + i]+", "+(P[i] + dp[i]));
